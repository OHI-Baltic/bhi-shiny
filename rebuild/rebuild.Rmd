...
sql <- read_lines("dashboard/data/bhidb.sql") %>% 
  str_remove("\t") %>% 
  paste(collapse = "") %>% 
  str_split("(?<=;)") %>% 
  unlist()
...
```{r make datalayers table}
## don't know why there are so many layer names missing from layers.csv in baltic2019draft branch...
## baltic2019draft is actually the paper version ... there is a GH release for the stage matching the paper


## Read layers.csv from baltic2019draft folder (current assessment)
layers_2019 <- read_csv(sprintf("%s/%s/layers.csv", gh_raw_bhi, scenario_folder), col_types = cols()) %>%
  select(layer, name_2019 = name) %>%
  filter(!is.na(layer), layer != "")

## Read layers.csv from baltic folder (previous assessment with more complete names)
layers_baltic <- read_csv(sprintf("%s/baltic/layers.csv", gh_raw_bhi), col_types = cols()) %>%
  select(layer, name_baltic = name) %>%
  filter(!is.na(layer), layer != "")


## get layers in database for pressures and resilience dimensions
## these are the only layers that we need the names for, the status info is reported by sources
pres_res_layers <- unique(c(pressures$layer, resilience$layer))

## Merge with layer names, preferring baltic names when available, then 2019 names
dataLayers <- data.frame(layer = pres_res_layers) %>%
  left_join(layers_baltic, by = "layer") %>%
  left_join(layers_2019, by = "layer") %>%
  mutate(full_layer_name = case_when(
    !is.na(name_baltic) & name_baltic != "" ~ name_baltic,
    !is.na(name_2019) & name_2019 != "" ~ name_2019
  )) %>%
  mutate(full_layer_name = case_when(
      layer == "hab_oxygen_debt" ~ "Oxygen debt",
      layer == "hab_illegal_oil" ~ "Illegal oil",
      layer == "hab_bottom_trawl" ~ "Bottom trawling",
      layer == "po_atmos_pcdd" ~ "Atmospheric deposition of PCDD/F",
      layer == "res_biodiversity" ~ "Biodiversity status",
      TRUE ~ full_layer_name
  )) |> 
  select(layer, full_layer_name)
